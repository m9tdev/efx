...
| `probe-escalate.mjs`       | `Async` failure homes, both step-08 blocks: a leaf-handled `HttpError` renders in place (page tag-map boundary untouched) while the SAME failure escalates the open-form block to its catch-all banner; the tag-map fetch loop recovers on a dep change with no reset while the open form needs its boundary's reset; leaf `retry` shows the loading arm (waiting → initial) then re-fails retryable; an unmatched `RateLimited` rides the residual to the page tag map (unwrapped payload); controls survive every swap. Prints PASS/FAIL, sets `process.exitCode`. |
...
| `probe-savebutton.mjs`     | The pending → run → settle handler pattern (#160/#161) in a real event loop, three phases: (1) click save — the handler swaps its own button for a spinner, then completes past the 600ms await (button returns, `saved` = 1); (2) save (fails) — the typed HttpError reaches the Catch after the self-triggered re-render; (3) ↻ reset while saving — the owner teardown interrupts the in-flight handler, no stale write lands in the fresh instance. Prints PASS/FAIL, exits 1 on failure.                                                                        |
