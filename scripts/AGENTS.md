...
| `probe-escalate.mjs` | `Async` failure homes (tag-map `failure` arm): a leaf-handled `HttpError` renders in place with the page boundary untouched; the fetch loop stays live (good id recovers, no reset); leaf `retry` re-runs the fetch; an unmatched `RateLimited` rides the residual to the page `Catch` tag map (unwrapped payload); controls survive the swap and boundary reset recovers. Prints PASS/FAIL, sets `process.exitCode`. |
