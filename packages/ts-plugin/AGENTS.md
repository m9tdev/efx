...
| `src/hint-text.ts`                | `hintText(hint)` — reads an inlay hint's label across the shapes different TS versions use (`hint.text` string, `hint.text` parts, `hint.displayParts`), returning the first non-empty. Plus `SUPPRESS_RE`, the `_tag`/`_props`/`_children`/`_name`/`_read` regex. Pure + unit-tested so the filter doesn't need a tsserver.                                                                                                                                                                                                                                                                                                                                                                                                    |
| `src/service-proxy.ts`            | `pluginFactory` — instantiates the shared LanguagePlugin via `createVerrexLanguagePlugin<string>(identity)`, builds Volar's `createLanguageServicePlugin` (capturing the session `Language` through its `setup(language)` hook), then wraps the resulting `LanguageService` in a Proxy with a few method overrides (filter `verrex`'s `h.ts` from definition results, JSX tag-pair document highlights, compiler diagnostics appended in `getSemanticDiagnostics` (from `VerrexVirtualCode.diagnostics`, source offsets, code 90001), `_tag`/`_props`/`_children`/`_name` inlay-hint filter, reference dedup + sort). Resolves the per-`.vx` `VerrexVirtualCode` from `language.scripts` when it needs `jsxRanges` or `source`. |
...
- **`provideInlayHints`** — `.vx`-only filter. Volar gives us all
  ...
  `_children`), `_read:` for `h.reader`'s wrapper (every reactive JSX
  expression), and the `_name:` label for `Component.make`'s
  ...
  `SUPPRESS_RE`. Bare `name:`/`read:` are deliberately NOT matched —
  common user parameters; only the underscore forms are suppressed.
