...
const counterEfx = join(demoRoot, "src/Counter.vx")
...
async function main() {
  ...
  const mainEfx = join(demoRoot, "src/main.vx")
  const todosEfx = join(demoRoot, "src/Todos.vx")
  ...
  const efxRefs = refs.filter(r => r.file?.endsWith(".vx"))
  const tsRefs = refs.filter(r => r.file?.endsWith(".ts") && !r.file?.endsWith(".d.ts"))
  if (efxRefs.length >= 3) {
    console.log(`   PASS: ${efxRefs.length} references in .vx files`)
  } else if (efxRefs.length > 0) {
    console.log(`   PARTIAL: Only ${efxRefs.length} .vx reference (expected at least 3: definition + import + usage)`)
  ...
  const efxFiles = files.filter((f) => f.endsWith(".vx"))
  console.log(`   Project has ${files.length} files, ${efxFiles.length} .vx files`)
  if (efxFiles.length > 0) {
    console.log("   .vx files:", efxFiles.map((f) => f.split("/").pop()).join(", "))
  } else {
