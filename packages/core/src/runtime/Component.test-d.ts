...
declare const liveBody: (props: {
  id: string
}) => Generator<never, View<HttpError>, never>
...
assertEquals<
  typeof _proplessFolded,
  Effect.Effect<View<never>, HttpError, HttpService>
>()
...
const Row = Component.make(
  <T>(props: { item: T; render: (item: T) => string }) =>
    Effect.succeed(someView),
  "Row",
)
