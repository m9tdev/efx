# `@efx/vite-plugin` — Vite integration for `.vx`
...
- `EFX_RE` — for the `transform` hook (`/\.vx(?:\?[^.]*)?$/`):
  matches the path with optional query, used to gate transforms.
- `EFX_PATH_RE` — for the `load` hook (`/\.vx$/`): matches the
  path *after* `id.split("?")[0]`, so no query handling needed.
- `EFX_URL_RE` — for the URL middleware (`/\.vx(\?.*)?$/`):
