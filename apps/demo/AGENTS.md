...
| `AsyncEscalate.vx` | `Async` failure homes (step 08): the `failure` arm picks where each error lives — per call site (function = all at the leaf, step 03; omitted = all escalate) and, live here, **per tag**. The thunk fails with `HttpError \| RateLimited` (a demo-local second tag). `HttpError` is handled **at the leaf** — the fetch loop stays live (another id recovers with no boundary involved; the handler's `retry` re-runs the same fetch); `RateLimited` rides the residual (`View<RateLimited>`, literally the `Exclude`) to a page-level `Catch` **tag map** that gets it unwrapped (`e.retryAfter` typed) and whose `reset` re-runs construction. The id buttons live outside the boundary so they survive the fallback swap. Deleting the `Catch` is a compile error at the demo mount naming only `RateLimited`. Exercised end-to-end by `scripts/probe-escalate.mjs`. |
