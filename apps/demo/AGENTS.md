...
| `CatchDemo.vx` | The `Catch` error boundary (step 07), both forms: a function handler (catch-all over a `Crasher` whose click fails) and an object tag-map (`{ HttpError }` over a `BadRequest` that fails construction with a typed error, routed + unwrapped). Discharges to `Effect<View, never, Scope>`; `reset` re-runs construction. Exercised end-to-end by `scripts/probe-catch.mjs`. |
